...
var _resultsViewJs = require("./views/resultsView.js"); // Note: Capitalized class name convention
...
const renderResultsAndCovers = async function() {
    try {
        ...
        const validCovers = await _modelJs.validateCovers();
        // Render the results
        (0, _resultsViewJsDefault.default).render(_modelJs.state.search.results, validCovers);
    } catch (error) {
...
const displayInitialResults = async function() {
    try {
        ...
        _sortViewJs.showSpinner();
        await _modelJs.loadSearchResults(query);
        // Display the total results found in the sort panel
        _sortViewJs.changeSortingMethodContent(_modelJs.state.search.resultsFound);
        await renderResultsAndCovers();
        _sortViewJs.hideSpinner();
    } catch (err) {
        console.log(err);
    }
};
const filterSortResults = async function() {
    try {
        // Load search results from the model
        _sortViewJs.showSpinner();
        await _modelJs.loadSearchResults(query, sortMethod, currentPage, language);
        await renderResultsAndCovers();
        _sortViewJs.hideSpinner();
    } catch (err) {
        console.log(err);
...
const controlPagination = async function(page) {
    try {
        _sortViewJs.showSpinner();
        // Load search results from the model
        await _modelJs.loadSearchResults(query, sortMethod, page);
        await renderResultsAndCovers();
        _sortViewJs.hideSpinner();
    } catch (err) {
        console.log(err);
...
const updatePage = function() {
    ...
    _paginationViewJs.calculatePage(event, _modelJs.state.search.resultsFound, (0, _configJs.RESULTS_PER_PAGE));
    currentPage = _paginationViewJs.pageCounter;
    console.log("Current Page:", currentPage);
    // Display results for the new page
    controlPagination(currentPage);
};
const init = function() {
    (0, _searchViewJsDefault.default).addHandlerSearch(displayInitialResults);
    _sortViewJs.sortListen(sortCallBack);
    _sortViewJs.sortLanguage(sortLanguageCallBack);
    _paginationViewJs.paginationControl(updatePage);
};
init();

},{"./model.js":"Y4A21","./views/resultsView.js":"cSbZE","./views/searchView.js":"9OQAM","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","./views/sortView.js":"hucv9","./config.js":"k5Hzs","./views/paginationView.js":"6z7bi"}],"Y4A21":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "state", ()=>state);
parcelHelpers.export(exports, "loadSearchResults", ()=>loadSearchResults);
parcelHelpers.export(exports, "validateCovers", ()=>validateCovers);
var _configJs = require("./config.js");
const state = {
    book: {},
    search: {
...
const loadSearchResults = async function(query, sortMethod, page, language) {
    console.log(`${(0, _configJs.API_URL)}${query}${(0, _configJs.API_URL_FIELDS)}${sortMethod ? sortMethod : ""}${(0, _configJs.API_URL_LIMIT)}${(0, _configJs.RESULTS_PER_PAGE)}&page=${page}${language ? `&lang=${language}` : ""}`);
    try {
        const response = await fetch(`${(0, _configJs.API_URL)}${query}${(0, _configJs.API_URL_FIELDS)}${sortMethod ? sortMethod : ""}${(0, _configJs.API_URL_LIMIT)}${(0, _configJs.RESULTS_PER_PAGE)}&page=${page}${language ? `&lang=${language}` : ""}`);
        if (!response.ok) throw new Error("Network data was not ok.");
...
const validateCovers = async function() {
        ...
        const validateBookCovers = async (book)=>{
            ...
            for (const key of book.editionKey){
                try {
                    ...
                    if (response.ok) validKeys.push(key);
                } catch (error) {
                    console.error(`Error validating cover for edition key ${key}:`, error);
                }
                if (validKeys.length === (0, _configJs.BOOK_EDITION_LENGTH)) break;
            }
            return validKeys;
        };
...
parcelHelpers.export(exports, "API_URL_FIELDS", ()=>API_URL_FIELDS);
parcelHelpers.export(exports, "RESULTS_PER_PAGE", ()=>RESULTS_PER_PAGE);
parcelHelpers.export(exports, "API_URL_LIMIT", ()=>API_URL_LIMIT);
parcelHelpers.export(exports, "BOOK_EDITION_LENGTH", ()=>BOOK_EDITION_LENGTH);
parcelHelpers.export(exports, "TOP_RATED", ()=>TOP_RATED);
parcelHelpers.export(exports, "EDITION_DESCENDING", ()=>EDITION_DESCENDING);
parcelHelpers.export(exports, "EBOOKS", ()=>EBOOKS);
parcelHelpers.export(exports, "WANT_TO_READ", ()=>WANT_TO_READ);
parcelHelpers.export(exports, "CURRENTLY_READING", ()=>CURRENTLY_READING);
const API_URL = `https://openlibrary.org/search.json?q=`;
const API_URL_FIELDS = `&fields=*`;
const RESULTS_PER_PAGE = 10;
const API_URL_LIMIT = `&limit=`;
const BOOK_EDITION_LENGTH = 4;
const TOP_RATED = "ratings_sortable desc";
const EDITION_DESCENDING = "edition_count desc";
const EBOOKS = "ebook_access desc";
const WANT_TO_READ = "Want_to_read_count desc";
const CURRENTLY_READING = "currently_reading_count desc";
...
class resultsView {
    ...
    _generateMarkup(validCovers) {
        return this._data.map((element, index)=>`<li class="results-ul-item">
            <span class="results-bookcover">
              <a href=""><img src="${element.cover ? `https://covers.openlibrary.org/b/olid/${element.cover}-L.jpg` : "./src/images/placeholder.jpg"}" alt="" /></a>
            </span>
            <div class="item-info">
              <div class="item-title"><a href="">${element.title}</a></div>
              <div class="item-author">by ${element.authorName}</div>
              <div class="published-date">First published in ${element.firstPublishedDate}</div>
              <span class="item-editions">
                <span class="editions-info">
                  <a href="">${element.editions} editions</a> in ${element.languages.split(" ").length} ${element.languages.split(" ").length > 1 ? "languages" : "language"}
                </span>
              </span>
              <div class="edition-list">
                <span id="result-${index}" class="editions-links">
                  ${validCovers[index].map((key)=>this._generateEditionMarkup(key)).join("")}
                </span>
              </div>
            </div>
            <div class="result-options">
              <a href="https://openlibrary.org/works/OL27448W/The_Lord_of_the_Rings?edition=key%3A/books/${element.cover}"><button class="borrow">Open Library</button></a>
              <button class="add-bookmarks-button">Add to My Books</button>
            </div>
          </li>`).join(""); // Join the array of HTML strings to form a single string
    }
}
...
class searchView {
    ...
    getQuery() {
        const query = this._parentElement.querySelector(".searchField").value.split(" ").join("+");
        this._clear();
        return query;
    }
    _clear() {
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"hucv9":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "showSpinner", ()=>showSpinner);
parcelHelpers.export(exports, "hideSpinner", ()=>hideSpinner);
parcelHelpers.export(exports, "sortLanguage", ()=>sortLanguage);
parcelHelpers.export(exports, "sortListen", ()=>sortListen);
parcelHelpers.export(exports, "changeSortingMethodContent", ()=>changeSortingMethodContent);
...
const resultsPanelContainer = document.querySelector(".results-panel-container");
...
const sortLanguage = function(callback) {
...
const sortListen = function(callback) {
...
const changeSortingMethodContent = function(data) {
...
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "previous", ()=>previous);
parcelHelpers.export(exports, "next", ()=>next);
parcelHelpers.export(exports, "pageCounter", ()=>pageCounter);
parcelHelpers.export(exports, "paginationControl", ()=>paginationControl);
parcelHelpers.export(exports, "calculatePage", ()=>calculatePage);
const previous = document.querySelector(".previous");
const next = document.querySelector(".next");
let pageCounter = 1;
const paginationControl = function(callback) {
...
const calculatePage = function(event, numFound, RESULTS_PER_PAGE) {
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["f0HGD","aenu9"], "aenu9", "parcelRequire3c77")
